class Game < ApplicationRecord
  ...
  def game_action
    ...
    all_players = find_players[2..-1] + find_players[0..1] unless stage
  ...
  def tie_game(winners)
    split_pot(winners)
    ActionCable.server.broadcast "room_channel", notification: "<div class='winner'>#{winners.map(&:username).join(" and ")} split the
